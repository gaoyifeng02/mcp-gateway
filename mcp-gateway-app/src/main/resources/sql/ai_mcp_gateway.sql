...
CREATE TABLE `mcp_gateway_tool` (
  ...
  `tool_id` bigint NOT NULL COMMENT '工具ID（业务主键）',
  `tool_name` varchar(128) NOT NULL COMMENT 'MCP工具名称（如：JavaSDKMCPClient_getCompanyEmployee）',
  `tool_type` varchar(32) NOT NULL DEFAULT 'function' COMMENT '工具类型：function-函数调用，resource-资源访问',
  `tool_description` varchar(512) DEFAULT NULL COMMENT '工具描述',
  `tool_version` varchar(16) NOT NULL DEFAULT '1.0.0' COMMENT '工具版本',
  `protocol_id` bigint DEFAULT NULL COMMENT '协议ID（关联协议配置表，如：mcp_protocol_http.id）',
  `protocol_type` varchar(32) NOT NULL DEFAULT 'http' COMMENT '协议类型：http、dubbo、rabbitmq等',
  `status` tinyint(1) NOT NULL DEFAULT '1' COMMENT '状态：0-禁用，1-启用',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_gateway_tool` (`gateway_id`, `tool_id`),
  UNIQUE KEY `uk_tool_name` (`gateway_id`, `tool_name`),
  KEY `idx_gateway_id` (`gateway_id`),
  KEY `idx_protocol_id` (`protocol_id`),
  KEY `idx_protocol_type` (`protocol_type`),
  KEY `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='MCP网关工具表';

LOCK TABLES `mcp_gateway_tool` WRITE;
/*!40000 ALTER TABLE `mcp_gateway_tool` DISABLE KEYS */;

INSERT INTO `mcp_gateway_tool` (`id`, `gateway_id`, `tool_id`, `tool_name`, `tool_type`, `tool_description`, `tool_version`, `protocol_id`, `protocol_type`, `status`, `create_time`, `update_time`)
VALUES
	(1,'gateway_001',1,'JavaSDKMCPClient_getCompanyEmployee','function','获取公司雇员信息','1.0.0',1,'http',1,'2026-01-02 13:10:19','2026-01-02 13:10:19');
...
CREATE TABLE `mcp_protocol_http` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID（协议ID）',
  `http_url` varchar(512) NOT NULL COMMENT 'HTTP接口地址',
  `http_method` varchar(16) NOT NULL DEFAULT 'POST' COMMENT 'HTTP请求方法：GET/POST/PUT/DELETE',
  `http_headers` text COMMENT 'HTTP请求头（JSON格式）',
  `timeout` int DEFAULT '30000' COMMENT '超时时间（毫秒）',
...
INSERT INTO `mcp_protocol_http` (`id`, `http_url`, `http_method`, `http_headers`, `timeout`, `retry_times`, `status`, `create_time`, `update_time`)
VALUES
	(1,'http://localhost:8701/api/v1/mcp/get_company_employee','POST','{\"Content-Type\": \"application/json\"}',30000,0,1,'2026-01-02 13:10:19','2026-01-02 13:10:19'),
	(2,'http://localhost:8701/api/v1/mcp/query-by-id','GET','{\"Content-Type\": \"application/json\"}',30000,0,1,'2026-01-02 13:10:19','2026-01-02 13:10:19');
